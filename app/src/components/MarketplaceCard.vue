<template>
  ...
  >
    ...
    <div class="text-center">
      <h2 class="text-lg font-semibold leading-6 text-blue-300">{{ item?.name }}</h2>
      <p class="text-base font-semibold leading-5 text-white">
        Prix en pessinos : {{ item?.price }} $
      </p>
    ...
    >
      <div class="bg-blue-950 p-4 rounded-lg shadow-lg text-center">
        ...
        <p class="text-white font-semibold">
          Êtes-vous sûr de vouloir acheter l'usine {{ item?.name }} ?
        </p>
        <div class="mt-4">
          <button
              class="bg-blue-400 text-white px-4 py-2 rounded-md mr-2"
              @click="confirmAction(item?._id, userId)"
          >
            Oui
          </button>
...
const confirmAction = (planetId: string, userId: string) => {
  console.log(userId)
  if (planetId) {
    marketplaceStore.buyOnePlanet(planetId, userId)
